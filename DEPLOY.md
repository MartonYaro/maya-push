...
- Ключи для данных не нужны — позиции парсятся из публичной выдачи App Store
- Жми **Apply** ещё раз

### 4) Жди ~3–5 минут первой сборки
Когда статус станет «Live» — открой URL из шапки сервиса. Кабинет, лендинг, SSE — всё должно работать.
...
## Английский лендинг (SEO для EU/USA)
`web/en/index.html` — это **сгенерированная** статическая EN-версия лендинга
(контент в HTML, не в JS-тогле, для индексации в Google + hreflang). Источник
правды — `web/index.html` (RU + переводы в `data-en`). После любой правки лендинга
перегенерируй EN-страницу и закоммить результат:
```bash
npm run build:en --prefix server   # → web/en/index.html
```
Скрипт: `server/scripts/build-en.mjs` (запекает data-en в текст, ставит lang/
canonical/og/hreflang, превращает переключатель языка в ссылки RU→/ , EN→/en/).
